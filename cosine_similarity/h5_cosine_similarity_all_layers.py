...
def _load_h5_examples_guess_only(path: str) -> dict:
    """Load HDF5 examples; keep examples with responses[0].embeddings_guess."""
    data = {}
    total_read_s = 0.0
    n_examples_read = 0
    with h5py.File(path, "r") as h5_file:
        ...
        for example_id in examples_group.keys():
            ...
            example_data = _read_h5_node(examples_group[example_id])
            elapsed = time.perf_counter() - t0
            total_read_s += elapsed
            print(f"[h5 read] {example_id!r} {_read_h5_node.__name__}: {elapsed:.6f}s")
    ...
    print(
        f"[h5 read] total {_read_h5_node.__name__} time for {path}: "
        f"{total_read_s:.6f}s ({n_examples_read} examples)"
    )
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            ...
            "embeddings_guess[4][last_layer], then plot by layer (HDF5 input)."
        )
    )
    ...
    print("Loading HDF5...")
    train_data = _load_h5_examples_guess_only(args.train_path)
    val_data = _load_h5_examples_guess_only(args.val_path)
